...
Original file is located at
    https://colab.research.google.com/drive/108Tce3M2XNt9FRjJjEosD3FzqIW_NaX2
"""
...
# function to creaye scrollable table
...
num_f = df.select_dtypes(include=[np.number])
summary_stats = num_f.describe().T
html_num = create_scrollable_table(summary_stats,'num_f','Summary statistics for numerical features')
display(HTML(html_num));

null_val =df.isnull().sum()
html_null = create_scrollable_table(null_val.to_frame(), "null_val", "Null Values in the dataset")

#Percantage of missing values for each feature
missing_per = (df.isnull().sum() / len(df)) * 100
html_missing_per = create_scrollable_table(missing_per.to_frame(), 'missing_per', "Percentage of missing values")

display(HTML(html_null + html_missing_per))

def drop_col(data_frame, threshold_percentage):
    """
    Drop columns in a DataFrame where the percentage of missing values exceeds the specified threshold.
    Parameters:
    ...
    updated_df = data_frame.drop(columns=columns_to_drop)

    print(f"Dropped columns with missing values exceeding {threshold_percentage}%: {columns_to_drop}")

    return updated_df

up_df= drop_col(df, 40)

up_df.shape

mu, sigma = stats.norm.fit(up_df['SalePrice'])

hist_data = go.Histogram(x=up_df['SalePrice'], nbinsx =50, name='Histogram', opacity =0.75, histnorm = 'probability density',  marker=dict(color='purple'))
# Calculate the normal distribution based on the fitted parameters
x_norm = np.linspace(up_df['SalePrice'].min(), up_df['SalePrice'].max(), 100)
...
# Create a Q-Q plot
qq_data = stats.probplot(up_df['SalePrice'], dist="norm")
...
dwelling_types = up_df['BldgType'].value_counts()
dwelling_prices = up_df.groupby('BldgType')['SalePrice'].mean()
...
zoning_prices = up_df.groupby('MSZoning')['SalePrice'].mean()
...
shape_prices = up_df.groupby('LotShape')['SalePrice'].mean()
contour_prices = up_df.groupby('LandContour')['SalePrice'].mean()
# Shape Prices
...
up_df['PropertyAge'] = up_df['YrSold'] - df['YearBuilt']

# Calculate Correlation between Property Age and Sale Price
age_price_corr =up_df['PropertyAge'].corr(df['SalePrice'])
...
fig9 = px.scatter(up_df, x='PropertyAge', y='SalePrice', title='Property Age vs Sale Price', color='PropertyAge', color_continuous_scale=px.colors.sequential.Purp)
...
categorical_columns = up_df.select_dtypes(include=['object', 'category']).columns
numerical_columns = up_df.select_dtypes(include=['int64', 'float64']).columns
...
"""run the same models now with PCA"""
...
# Create the GridSearchCV object
grid_search_mlp_pca = GridSearchCV(mlp, param_grid, scoring='neg_mean_squared_error', cv=5, n_jobs=-1, verbose=1)
...
grid_search_mlp_fe = GridSearchCV(mlp, param_grid, scoring='neg_mean_squared_error', cv=5, n_jobs=-1, verbose=1)
# Fit the model on the training data
...
df_test_pre = pipeline_fe.transform(df_test)

#xgboost submission
y_xgboost = np.exp(grids_fe['XGBoost'].predict(df_test_pre))
...
#regression forest  submission
y_rf = np.exp(grids_fe['RandomForest'].predict(df_test_pre))
...
#mlp submission
y_mlp = np.exp(grid_search_mlp_fe.predict(df_test_pre))
